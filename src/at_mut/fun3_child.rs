...
impl<I, T, F, M> AtMut<<D3 as Dim>::ChildDim, T> for FunMutAt3Child<'_, I, T, F, M>
where
    F: for<'b> Fn(&'b I, [usize; 3]) -> &'b T,
    M: for<'b> FnMut(&'b mut I, [usize; 3]) -> &'b mut T,
{
    ...
    fn at_mut(&mut self, [i, j]: <<D3 as Dim>::ChildDim as Dim>::Idx) -> &mut T {
        let idx = [self.c, i, j];
        (self.m)(self.data, idx)
    }

    fn try_at_mut(&mut self, idx: <<D3 as Dim>::ChildDim as Dim>::Idx) -> Option<&mut T> {
